...
export function HotOrNotNightlifePage() {
  const navigate = useNavigate();
  const [isDialogOpen, setIsDialogOpen] = useState(false);
  const [searchQuery, setSearchQuery] = useState("");
  const [activeTab, setActiveTab] = useState<EventTab>("contestants");
  const [selectedStatuses, setSelectedStatuses] = useState<CompatibleEventStatus[]>([]);
  ...
  const [selectedEvent, setSelectedEvent] = useState<HotOrNotEvent | null>(null);
  const [isLeaderboardOpen, setIsLeaderboardOpen] = useState(false);

  // Use Redux hook for real data with NIGHTLIFE theme filtering
  const {
    // State
    contests,
    ...
    mode: "admin", 
    autoLoad: true,
    theme: "NIGHTLIFE" // Only get NIGHTLIFE contests
  });
    ...
    const fetchData = async () => {
      try {
        if (activeTab === "contestants") {
          await fetchNominationCandidates({ theme: "NIGHTLIFE" });
        } else {
          await fetchContests({ forceRefresh: true, theme: "NIGHTLIFE" });
        }
      } catch (error) {
  ...
  const eventsData: HotOrNotEvent[] = useMemo(() => {
    return contests.map((contest: HotOrNotContest) => {
      ...
      const endsAt = contest.startDate && contest.durationHours 
        ? new Date(new Date(contest.startDate).getTime() + contest.durationHours * 60 * 60 * 1000).toISOString()
        : new Date().toISOString(); // Fallback to current date

      // Fix: Map status to compatible type (exclude "draft")
      const mappedStatus: CompatibleEventStatus = contest.status === "DRAFT" 
  ...
  // Calculate statistics based on real data (only NIGHTLIFE contests)
  ...
  const totalVotes = contests.reduce(
    (sum, contest) => sum + contest.contestants.reduce(
  ...
  const hotOrNotCards: CardData[] = [
    {
      ...
      change: {
        description: "Nightlife contest categories",
      },
  ...
  const handleNominateConfirm = async (
    ...
    try {
      ...
      const result = await assignNominees(campaignId, { postIds: contestantIds });
      ...
      if (result) {
        ...
        await fetchNominationCandidates({ theme: "NIGHTLIFE", forceRefresh: true });
                    ...
                    <div className="flex flex-col sm:flex-row gap-4 items-center sm:items-center justify-between mb-6">
                      ...
                      <div>
                        <div className="flex items-center gap-2">
                          ...
                          <Button
                            onClick={() => handleNominateClick(selectedContestants)}
                            className="bg-[#5041D0] hover:bg-[#5041D0]/90 text-white h-11"
                            disabled={selectedContestants.length === 0 || nominationCandidates.length === 0}
                          >
                            Nominate
                          </Button>
                        </div>
